...
""".replace(b"\n", b"\r\n"))
    ...
    def test_chunked_request_body_invalid_size(self):
        # Only hex digits are allowed in chunk sizes. Python's int() would
        # accept the underscore separator, which a proxy in front of us would
        # not, and that mismatch is a request smuggling vector.
...
""".replace(b"\n", b"\r\n"))
    ...
    def test_chunked_request_body_leading_plus_size(self):
        # int(x, 16) also accepts a leading plus sign.
        with ExpectLog(gen_log, '.*invalid chunk size'):
            self.stream.write(b"""\
POST /echo HTTP/1.1
Transfer-Encoding: chunked
...
""".replace(b"\n", b"\r\n"))
    ...
    def test_invalid_content_length_4(self):
        # A negative Content-Length used to be read as a zero-length body,
        # leaving the rest of the request to be parsed as a second one.
        with ExpectLog(gen_log, '.*Only integer Content-Length is allowed'):
            self.stream.write(b"""\
POST /echo HTTP/1.1
Content-Length: -10
